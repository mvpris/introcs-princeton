public class app2412 {
    public static double function(int n, double p) {
        return 1.0 - Math.pow(1.0 - Math.pow(p, n), n);
    }

    public static boolean percolates(double p1, double p2) {
        double error = 1E-4;
        return Math.abs(p1 - p2) <= error;
    }

    public static double estimate(int n, double p, int trials) {
//      Generate trials random n-by-n systems;
//      return empirical percolation probability estimate.
        int count = 0;
        for (int t = 0; t < trials; t++) { // Generate one random n-by-n boolean matrix.
            boolean[][] isOpen = PercolationVertical.random(n, p);
            if (PercolationVertical.percolates(isOpen)) count++;
        }
        return (double) count / trials;
    }

    public static void main(String[] args) {
        int n = Integer.parseInt(args[0]);
        double p = Double.parseDouble(args[1]);
        int trials = Integer.parseInt(args[2]);
        double p1 = estimate(n, p, trials);
        double p2 = function(n, p);
        boolean percolates = percolates(p1, p2);
        System.out.println("p1 = " + p1);
        System.out.println("p2 = " + p2);
        System.out.println("percolates = " + percolates);
    }
}
